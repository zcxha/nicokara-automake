...
def _line_to_karaoke_text(line: dict[str, Any]) -> tuple[float, float, str] | None:
    ...
    karaoke_parts = []
    for index, (text, start, end) in enumerate(segments):
    ...
    return line_start, line_end, "".join(karaoke_parts)
...
def payload_to_ass_text(
    ...
    *,
    ...
    for line in payload.get("lines", []):
        rendered = _line_to_karaoke_text(line)
        if rendered is None:
            continue
        start, end, karaoke_text = rendered
        events.append(
            "Dialogue: 0,"
